...
struct DessertListView: View {
    ...
    private var dessertList: some View {
        List {
            ForEach(viewModel.sectionTitles, id: \.self) { letter in
                // Only create a section if there are desserts to show
                if viewModel.searchResults(for: letter).isEmpty == false {
    ...
    private func dessertSection(letter: String) -> some View {
        Section(header: Text(letter)) {
            ForEach(viewModel.searchResults(for: letter)) { dessert in
